// TODO: Need info from the API
const scheduleTitles = ['ID', 'Start time', 'End time']
const roomsTitles = ['ID', 'Name', 'Code', 'Floor', 'State']
const doctorListTitles = ['ID', 'Name', 'Mail', 'Phone', 'RH']
const specialtyTitles = ['ID', 'Name', 'Description', 'State']
const usersTitles = ['ID', 'Document', 'Rol', 'Name', 'Email', 'Phone', 'Gender']

export { scheduleTitles, roomsTitles, doctorListTitles, specialtyTitles, usersTitles }
